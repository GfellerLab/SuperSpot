...
## Import data
so = readRDS("/work/FAC/FBM/LLB/dgfeller/scrnaseq/mteleman/SuperSpot/VisiumMouseBrain/cortex_with_membership.rds"  )
SpatialDimPlot(so)

spotPosition <- GetTissueCoordinates(so)
effective.gammas <- c()

for (g in 1:25){
  n.pc = 1:30 # number of first PC to use
  k.knn = 6 # number of neighbors to connect to each spot
  MC <- SCimplify_SpatialDLS(X =  GetAssayData(so,assay = "Spatial",layer = "count"),
  ...
  so@meta.data[,str_c("MC_membership_",g)] <- MC$membership %>% as.character()
  MC$cell_type <- supercell_assign(clusters = so@meta.data$layer_guess,
                                   supercell_membership = MC$membership,
                                   method = "absolute")
  MC.ge <- superspot_GE(MC = MC,
                        ge =  GetAssayData(so,assay = "Spatial",layer = "count"),
  ...
  tissue_position  <- read.csv("/work/FAC/FBM/LLB/dgfeller/scrnaseq/mteleman/SuperSpot/VisiumMouseBrain/spatial/tissue_positions_list.csv",header = F)
  ...
  image <- png::readPNG(source = "/work/FAC/FBM/LLB/dgfeller/scrnaseq/mteleman/SuperSpot/VisiumMouseBrain/spatial/tissue_lowres_image.png")
  scale.factors <- Read10X_ScaleFactors(filename = "/work/FAC/FBM/LLB/dgfeller/scrnaseq/mteleman/SuperSpot/VisiumMouseBrain/spatial/scalefactors_json.json")
  ...
  saveRDS(MC.seurat,file = paste0("/work/FAC/FBM/LLB/dgfeller/scrnaseq/mteleman/SuperSpot/VisiumMouseBrain/MC_seurat_g",g,".rds"))
}
