...
## Import data
so = readRDS("./SuperSpot/VisiumMouseBrain/cortex_with_membership.rds"  )
SpatialDimPlot(so)
...
for (g in 1:25){
  ...
  tissue_position  <- read.csv("./SuperSpot/VisiumMouseBrain/spatial/tissue_positions_list.csv",header = F)
  ...
  image <- png::readPNG(source = "./SuperSpot/VisiumMouseBrain/spatial/tissue_lowres_image.png")
  scale.factors <- Read10X_ScaleFactors(filename = "./SuperSpot/VisiumMouseBrain/spatial/scalefactors_json.json")
  ...
  saveRDS(MC.seurat,file = paste0("./SuperSpot/VisiumMouseBrain/MC_seurat_g",g,".rds"))
}
