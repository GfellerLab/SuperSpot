...
distinctive_colors2 <- c('#E59CC4', '#53A85F', '#F1BB72', '#F3B1A0', '#D6E7A3', '#57C3F3', '#476D87',
                      '#E95C59', '#E5D2DD', '#AB3282', '#23452F', '#BD956A', '#8C549C', '#585658',
                      '#9FA3A8', '#E0D4CA', '#5F3D69', '#58A4C3', "#b20000", '#E4C755', '#F7F398',
                      '#AA9A59', '#E63863', '#E39A35', '#C1E6F3', '#6778AE', '#91D0BE', '#B53E2B',
                      '#712820', '#DCC1DD', '#CCE0F5', '#CCC9E6', '#625D9E', '#68A180', '#3A6963',
                      '#968175')[1:12] 

distinctive_colors3 <- c('#E59CC4', '#53A85F', '#F1BB72', '#F3B1A0', '#D6E7A3', '#57C3F3', "#F8766D",
...
ggplot2::ggplot(plt.df) + 
  ...
  scale_color_manual(values = distinctive_colors2) +
  scale_y_reverse()+
...
SpatialDimPlotSC(original_coord = spotPosition,
                 ...
                 polygons_col = "polygons",
                 meta_data = metad,alpha_hull = 0.5,spot.color = distinctive_colors2)+
  theme(panel.background = element_rect(fill="white"),
        plot.background = element_rect(fill="white"),
        panel.grid = element_line(color="white"),
        axis.text = element_text(color="black"),
...
ggplot(umap_sized.df) +geom_point(aes(x = umap_1, y = umap_2,size = superspot_size,fill = ident),shape = 21, stroke = 0.5)+ 
     scale_fill_manual(values=distinctive_colors2)+theme(axis.ticks.length=unit(.25, "cm"),panel.background = element_rect(fill="white"), plot.background = element_rect(fill="white"),text = element_text(size = 14),
                                                         axis.text = element_text(color="black"),
                                                         axis.title = element_text(color="black"),
...
mc.markers <-  FindAllMarkers(MC.seurat,
                                    only.pos = TRUE,
                                    #min.pct = 0.25,
...
DotPlot(MC.seurat,features = mc.top.markers$gene %>% unique()) + RotatedAxis()
```
...
```{r}
sp.top.markers <- readxl::read_xlsx("/Users/admin/Documents/These/data/PancreasCosMx/spot_top_markers.xlsx")
```

```{r}
tpr <- c()
for (cl in unique(sp.top.markers$cluster)){
  sb.spot <- subset(sp.top.markers, cluster == cl)
  sb.metaspot <- subset(mc.top.markers, cluster == cl)
  com.genes <- intersect(sb.spot$gene,sb.metaspot$gene)
  score <- length(com.genes)/length(sb.spot$gene)*100
  tpr <- c(tpr,score)
}
names(tpr) <- unique(sp.top.markers$cluster)
tpr
```

```{r}
for (cl in unique(sp.markers$cluster)){
  sb.spot <- subset(sp.markers, cluster == cl)%>%
  filter(p_val_adj < 0.05)
  sb.metaspot <- subset(mc.markers, cluster == cl)%>%
  filter(p_val_adj < 0.05)
  plot(ggvenn::ggvenn(list(spot = sb.spot$gene, metaspot = sb.metaspot$gene),show_percentage = F,text_size = 10,auto_scale = T),main = cl)
}
```
...
## Subsampling
```{r}
cell.ids <- sample(colnames(mtx.flt),ncol(mtx.flt)/2.6)
mtx.sb <- mtx.flt[,cell.ids]
metad.sb <- metad[cell.ids,]
sb.so <- CreateSeuratObject(mtx.sb,meta.data = metad)
```

```{r}
sb.so <- NormalizeData(sb.so)

sb.so <- ScaleData(sb.so)

sb.so <- FindVariableFeatures(sb.so)

sb.so <- RunPCA(sb.so)

sb.so <- RunUMAP(sb.so, dims = 1:30)

DimPlot(sb.so, reduction = "umap", group.by = "cell_type")
```

```{r}
Idents(sb.so) <- "cell_type" 
levels(sb.so) <- sort(levels(sb.so))

VlnPlot(sb.so,"nFeature_RNA")+NoLegend()

#Compute marker genes
sb.markers <-  FindAllMarkers(sb.so,
                                    only.pos = TRUE,
                                    #min.pct = 0.25,
                                    logfc.threshold = 0.25 ) %>%
  filter(p_val_adj < 0.05)

sb.top.markers <- sb.markers %>%
   group_by(cluster) %>%
    slice_max(n = 20, order_by = avg_log2FC)

DotPlot(sb.so,features = sb.top.markers$gene %>% unique()) + RotatedAxis()
```
...
for (cl in unique(sp.top.markers$cluster)){
  sb.spot <- subset(sp.top.markers, cluster == cl)
  sb.subsamp <- subset(sb.top.markers, cluster == cl)
  com.genes <- intersect(sb.spot$gene,sb.subsamp$gene)
  score <- length(com.genes)/length(sb.spot$gene)*100
  sb.tpr <- c(sb.tpr,score)
}
names(sb.tpr) <- unique(sp.top.markers$cluster)
sb.tpr
mean(sb.tpr)
```

```{r}
test <- readxl::read_xlsx("/Users/admin/Documents/GitHub/SuperSpot/results_MC_spl.xlsx")
plot(1:20, test$min, type = 'b', col = 'blue', ylim = range(c(test$min)), ylab = 'Recquired Time')
par(new = TRUE)
plot(1:20, test$mem_alloc, type = 'b', col = 'red', ylim = range(test$mem_alloc), axes = FALSE, xlab = '', ylab = '')
axis(side = 4)
```


```{r}
test2 <- readxl::read_xlsx("/Users/admin/Documents/GitHub/SuperSpot/results_MC.xlsx")
plot(1:20, test2$min, type = 'b', col = 'blue', ylim = range(c(test2$min)), ylab = 'Recquired Time')
par(new = TRUE)
plot(1:20, test2$mem_alloc, type = 'b', col = 'red', ylim = range(test2$mem_alloc), axes = FALSE, xlab = '', ylab = '')
axis(side = 4)
```
...
# Step 2: Filter cells belonging to the cell type of interest
cells_of_interest <- WhichCells(object = nano.obj, idents = "Acinar.2")
...
# Fetch the expression data for the gene of interest
expression_data <- FetchData(object = nano.obj, vars = gene_of_interest, cells = cells_of_interest)
