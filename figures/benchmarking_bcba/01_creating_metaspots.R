...
## Import data
BCBA.so = Seurat::Load10X_Spatial("./SuperSpot/BCBA/"  )
SpatialDimPlot(BCBA.so)
bcba.md <- read.csv("./SuperSpot/BCBA/label.csv",header = T,sep = ";") %>% column_to_rownames("X")
...
for (g in 1:25){
  ...
  image <- png::readPNG(source = "./SuperSpot/BCBA/spatial/tissue_lowres_image.png")
  scale.factors <- Read10X_ScaleFactors(filename = "./SuperSpot/BCBA/spatial/scalefactors_json.json")
  ...
  saveRDS(MC.seurat,file = paste0("./SuperSpot/BCBA/Output/MC_seurat_g",g,".rds"))
}
