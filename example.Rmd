...
Because we wanted that every metaspot contains only one cell type and we split them, it created metaspots with gaps. To overcome this, we split them again based on if there are still connected or not in the KNN.
```{r}
...
DimPlot(MC.seurat, reduction = "umap", group.by = "Main_molecular_cell_type")+NoLegend()
```
